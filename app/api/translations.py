...
async def create_translation(request: TranslateRequest):
    ...
    if request.deployment_location:
        ...
        db.save_deployment_record(dep)
        deployments.append(dep)
    
    # ── 5. Build provenance record ────────────────────────────────────────────
    prov_record = build_provenance_record(unit, deployments)
    unit.prov_entity_id = prov_record.entities[1].id if len(prov_record.entities) > 1 else None
    db.save_translation_unit(unit)
    db.save_provenance_record(prov_record)
    ...
    db.save_xliff(xliff_doc_id, xliff_xml)
...
async def list_translations(
    ...
    db = get_db()
    units = db.list_translation_units(source_language, target_language, method, status, limit)
...
async def record_deployment(
    ...
    # Update provenance
    all_deps = db.get_deployments_for_unit(unit_id)
    prov_record = build_provenance_record(unit, all_deps)
    db.save_provenance_record(prov_record)
    db.save_prov_json(prov_record.bundle_id, to_prov_json(prov_record))
...
async def mark_reviewed(
    ...
    reviewer = db.get_or_create_agent(reviewer_name, "Person")
